...
test('«Скрыть» у строки защиты данных держится до следующего запуска, а не до смены вкладки', async ({ page }) => {
  ...
  // «Данные только на этом устройстве» (теперь «Нет синхронизации и облачной
  // копии», 25.09) возвращалась уже после перехода на
  // другую вкладку и обратно.
  await openApp(page, '/');
  const line = page.getByText('Нет синхронизации и облачной копии', { exact: true });
