        ...
        <ErrorBoundary>
          ...
          <div className="fixed inset-0 flex flex-col overflow-hidden bg-bg">
            ...
            {/* Полоса под плавающую кнопку — ВНУТРЕННИМ отступом ленты, а не
                внешним. Переменную выставляет сама кнопка (Fab.tsx), на
                экранах без неё полоса нулевая.

                С 22 августа здесь стоял margin: лента физически укорачивалась
                на 68px, и под кнопкой оставался голый фон каркаса — чтобы
                кнопка не воровала тапы у того, что под неё попало (на
                «Сегодня» это были «отправить», микрофон, крестик подсказки).
                Лечение оказалось заметнее болезни. Восьмая часть экрана
                телефона стала пустотой: между концом ленты и таб-баром висел
                тёмный прямоугольник, которого владелец не заказывал, а строки
                списка обрезались его краем на полуслове — замерено, низ ленты
                668 при верхе таб-бара 736. Владелец: «это тёмное поле нужно
                убрать вообще, его не было».

                Теперь лента идёт до таб-бара, а место под кнопку держит
                нижний отступ самого экрана (Screen.tsx): фон непрерывен,
                ничего не обрезается, а в конце прокрутки контент по-прежнему
                заканчивается выше кнопки — последняя строка достижима. Ни
                padding у скроллера, ни распорка после обёртки не работают:
                контент экрана выше своей обёртки и вылезает из неё вниз. На промежуточных позициях под кнопкой проезжает
                контент, и это обычное поведение плавающей кнопки: чтобы
                нажать то, что под ней, человек прокручивает. */}
            <div
              id="app-scroll"
