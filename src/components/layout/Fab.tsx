...
export function Fab({ onClick, label }: Props) {
  ...
  // Пока кнопка на экране — лента держит под неё полосу внизу (её читает
  // внутренний отступ скролл-контейнера в App.tsx), и в конце прокрутки
  // контент заканчивается выше кнопки. Layout-эффект, а не обычный: полоса
  // появляется до отрисовки кадра, иначе на переходе виден скачок ленты.
  useLayoutEffect(() => {
    mounted += 1;
