...
// «Женские дни» — тоже не входят, ни сводкой, ни автозадачами раздела в
// list_tasks. Решение владельца 25.09 (задача 33) отменяет решение 17.08:
// экран раздела обещает «записи не уходят на сервер», а ответ облачной
// модели идёт через наш воркер к провайдеру открытым текстом.
...
export const TOOL_LABELS: Record<string, string> = {
  ...
  // Инструмента больше нет (25.09) — подпись только для следа старых ответов,
  // иначе под ними чип показывал бы машинное «cycle_summary».
...
async function runListTasks(args: Record<string, unknown>): Promise<ToolResult> {
  ...
  // Автозадачи «Женских дней» («скоро начнутся») — тоже данные цикла.
...
/** Системная приписка при выключенном доступе: иначе модель не знает ни
 *  где она, ни что есть «Данные», и на «что у меня по задачам?» отвечает
 *  «нет доступа» без подсказки — или пересказывает прошлые ответы из истории
 *  как сегодняшние факты. */
export function noDataSystemPrompt(): string {
  return [
    'Ты — ассистент органайзера LifeHearth. Доступа к записям пользователя в этом чате нет.',
    'На вопросы о его задачах, заметках, финансах и привычках отвечай, что для этого нужно включить «Данные» под полем ввода.',
    'Прежние ответы в истории чата — не текущие данные.',
  ].join(' ');
}
...
export function toolsSystemPrompt(): string {
  return [
    ...
    'Раздел «Женские дни» тебе недоступен — если спросят о цикле, прямо скажи, что ассистент его не читает.',
