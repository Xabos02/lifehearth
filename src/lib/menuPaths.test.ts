...
describe('маршруты в инструкциях', () => {
  // «Главная → …» — тем же сторожем: приглашение в семью вело через «＋», а на
  // экране того, у кого групп ещё нет, это с августа 2026 не кнопка, а
  // картинка над «Войти по приглашению».
  it('каждый шаг вида «Настройки → …» и «Главная → …» существует как подпись на экране', () => {
    const broken: string[] = [];
    files.forEach((file, i) => {
      ...
      for (const m of all[i].matchAll(/(?:Настройки|Главная)\s*→[^'"`\n]*/g)) {
