...
export interface Task extends BaseEntity {
  ...
  tags: string[];
  sortOrder: number;
  /** Разовое поручение, а не то, что живёт в проекте долго.
   *
   ...
   *  постоянное тонет в мелочи, а мелочь выглядит так же весомо. Пометка
   *  ставится руками, а не угадывается по сроку: дедлайн у постоянной задачи
   *  бывает не реже, чем у разовой, и догадка врала бы ровно на них.
   *
   *  undefined = обычная задача. Группировку целиком можно выключить
   *  (Settings.groupTemporary) — кто так не думает, тому она лишняя. */
  temporary?: boolean;
  // Сколько раз задачу отмечали «пропущена» (для статистики). undefined = 0.
  skippedCount?: number;
  /** Id следующего повтора, созданного при выполнении повторяющейся задачи.
   *  Нужен, чтобы снятие отметки (промах по чекбоксу — обычное дело) убирало
   *  и его: иначе оставались две задачи — исходная и «следующая». Без
   *  индекса, версию схемы не поднимает. */
...
/** Вид тренировки — фиксированный набор, а не свободный текст: по нему
 *  красятся точки в календаре и считается сводка «по видам». Подписи и
 *  цвета — features/health/workouts.ts. */
...
export interface Workout extends BaseEntity {
  ...
  type: WorkoutType;
  minutes: number;
  /** Дистанция в км — только у бега, ходьбы, вело, плавания; иначе null. */
  distanceKm: number | null;
  /** Как прошло: 1 легко · 2 нормально · 3 тяжело · 4 на пределе. null — не отмечено. */
  effort: 1 | 2 | 3 | 4 | null;
  note: string;
...
export interface Settings {
  ...
  gender?: 'female' | 'male';
  /** Цель по тренировкам в неделю (раздел «Здоровье»). undefined = 3 —
   *  ритм «через день», с которого начал владелец. Device-local, как тема. */
  ...
  /** Когда человек подтвердил, что сохранил ключ восстановления (ISO).
   *
   *  Пустое значение — не мелочь, а состояние «данные в облаке есть, а вернуть
   *  их нечем»: ключ существует в одном экземпляре, на этом устройстве, и
   *  второй копии нет ни у кого. Отметки не было вовсе, поэтому приложение не
   *  могло ни напомнить, ни предупредить при отключении обмена — а экран, где
   *  ключ сохраняют, назывался «Код для другого устройства» и человеком с
   *  одним телефоном закрывался не читая.
   *
   *  Ставится только после того, как человек вставил сохранённое обратно и оно
   *  совпало: скачанный файл ещё не значит сохранённый — он мог уйти пустым,
   *  не туда или на тот самый телефон, который потом и потеряется. */
  recoveryKeySavedAt?: string | null;
  /** Когда человек нажал «Принимаю» в окне «Что уходит с телефона» (ISO).
   *  Пусто — всё внешнее выключено, локальное работает (lib/consent.ts).
   *  Согласие даёт человек на этом телефоне: settings не синхронизируется, а
   *  копия его не уносит и при восстановлении не трогает (db/backup.ts). */
  consentAt?: string | null;
  /** Когда окно согласия закрыли без «Принимаю». Само оно показывается один
   *  раз; дальше — только по попытке включить внешнее или из настроек. */
  ...
  /** Почему не удалась — человеческим языком. Одной даты мало: «не удалось»
   *  одинаково выглядит и когда нет сети, и когда сервер отказал в доступе, а
   *  чинится это по-разному. Без причины владелец видел только «данные не
   *  появляются» и не мог понять, на чьей стороне беда. */
  ...
  /** Собирать ли временные задачи отдельной группой внутри проекта.
   *  undefined = да. Device-local, как и остальные настройки. */
...
export interface FamilyConfig {
  ...
  removedAt?: string;
  // Группу удалил владелец. Отличается от removedAt («исключили»): причина
  // уведомления другая, хотя поведение то же — переписка на устройстве
  // остаётся, но новые сообщения уже не придут никогда.
  groupDeletedAt?: string;
}
...
export interface LlmChat extends BaseEntity {
  ...
  // Доступ модели к данным приложения (этап 3). undefined — ВЫКЛЮЧЕН (решение
  // 25.09, задача 33): прочитанное уходит провайдеру открытым текстом, включает
  // человек. Читать только как `=== true`.
  dataTools?: boolean;
...
export interface SyncConfig {
  ...
  key: CryptoKey;
  enabled: boolean;
  // Курсор приёма: порядковый номер последней полученной записи — номер
  // ставит СЕРВЕР в момент прихода, часы устройств тут ни при чём. Нет поля
  // (конфиг старше этой правки) — читаем с нуля, один раз. См. lib/sync.ts.
  lastPullSeq?: number;
  // Прежний курсор приёма по времени правки. Больше не используется: запись,
  // пришедшая на сервер позже более свежей, по нему терялась навсегда.
  // Поле остаётся, чтобы старые конфиги читались как есть.
  lastPullAt: string;
  ...
  knownTables?: string[];
  // Случайное имя этого устройства для сервера. Не секрет: только чтобы сервер
  // не отдавал устройству его же записи и не будил его же сокет. Ставится при
  // первом обмене, если ещё нет.
  deviceId?: string;
