...
/** Предельная глубина дерева проектов: проект → подпроект → под-подпроект.
 *
 *  Решение владельца от 11.09.2026. Посчитано по разметке: уровень стоит
 *  20px из 343px полосы контента, третий проходит свободно (текст задачи
 *  225px), на четвёртом строка с датой и тегами начинает переноситься. Глубже
 *  приложение перестаёт помещаться в телефон. */
